...
class RetryHandler(private val maxRetries: Int) {

    fun retryRequest(exception: IOException, executionCount: Int): Boolean {
        Log.d("retryRequest", "exception:" + exception.javaClass + " executionCount=" + executionCount)
        var retry = true
        when {
            ...
            exceptionBlacklist.contains(exception.javaClass) -> // immediately cancel retry if the error is blacklisted
                retry = false
            exceptionWhitelist.contains(exception.javaClass) -> {
