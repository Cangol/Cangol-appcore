...
public class SocketThread implements Runnable {
    ...
    private static final int WRITE_DELAY_TIME = 1000;
    private static final int LINGER_TIME = 5000;
    private static final int CONNECT_TIME_OUT = 20 * 1000;
    private static final int RETRY_TIMES_SHORT = 3;
    private static final int RETRY_TIMES_LONG = 5;
    private String host;
    private int port;
    ...
    public SocketThread(String host, int port, boolean isLong, int timeout, SocketHandler socketHandler) {
        this.port = port;
        this.host = host;
        this.isLong = isLong;
    ...
    private void makeRequest() throws ClassNotFoundException, IllegalBlockingModeException, IllegalArgumentException,SocketTimeoutException, IOException {
        ...
        if (!Thread.currentThread().isInterrupted()) {
            ...
            if (isLong)socket.setSoLinger(true, LINGER_TIME);
            ...
            if (DEBUG)Log.d(TAG, "socket is " + (isLong ? "isLong" : "") + " connect.");
            if (!isLong) {
                handleSocketWrite();
    ...
    public  boolean handleSocketWrite() {
        ...
        try {
            ...
            if (DEBUG) Log.d(TAG,"handleSocketWrite "+e);
    ...
    public  boolean handleSocketRead(int timeout)  {
        ...
        try {
            ...
            if (DEBUG) Log.d(TAG,"handleSocketRead "+e);
            Object[]  response = {null,e};
            socketHandler.sendFailMessage(response);
            if(e instanceof  SocketTimeoutException){
    ...
    public synchronized  void disconnect()  {
        ...
        try {
            ...
            if (DEBUG)Log.d(TAG, "socket close.");
        } catch (IOException e) {
            if (DEBUG)Log.d(TAG, "disconnect Exception "+e.getMessage());
        }
    }
    ...
    public void run() {
        try {
            ...
            isConnecting = false;
            if (DEBUG)Log.d(TAG, "InterruptedException "+e.getMessage());
            disconnect();
        }catch (Exception e) {
            if (DEBUG)Log.d(TAG, ""+e.getCause());
    ...
    class SocketReadThread implements Runnable {
        ...
        @Override
        public void run() {
            while (!Thread.currentThread().isInterrupted()&&isConnecting) {
                handleSocketRead(timeout);
            }
        }
    }
