...
public class OpenUDID_manager implements ServiceConnection{
	...
	private List<ResolveInfo> mMatchingIntents; //List of available org.OpenUDID Intents
	private Map<String, Integer> mReceivedOpenUDIDs; //Map of OpenUDIDs found so far

	private final SharedPreferences mPreferences; //Preferences to store the org.OpenUDID
	...
	private void startService() {
		if (mMatchingIntents.size() > 0) { //There are some Intents untested
			...
			if (OpenUDID == null) //No org.OpenUDID was chosen, generate one
				generateOpenUDID();
			if (LOG) Log.d(TAG, "org.OpenUDID: " + OpenUDID);
	...
	/**
	 * The Method to call to get org.OpenUDID
	 * @return the org.OpenUDID
	 */
	...
	public static void sync(Context context) {
		...
		{
			//Get the list of all org.OpenUDID services available (including itself)
			manager.mMatchingIntents = context.getPackageManager().queryIntentServices(new Intent("org.org.OpenUDID.GETUDID"), 0);
			if (LOG) Log.d(TAG, manager.mMatchingIntents.size() + " services matches org.OpenUDID");
